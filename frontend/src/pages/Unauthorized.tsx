import { FiUsers, FiCloud, FiMusic, FiLock } from "react-icons/fi";
import Navbar from "../components/shared/NavbarFan";
  ...
  return (
    <div className="bg-gradient-to-br from-[#0f0f0f] via-[#1a1a1a] to-[#2a2a2a] text-white min-h-screen flex flex-col relative overflow-hidden">
      ...
      <main className="flex-grow flex flex-col justify-center items-center px-4 sm:px-6 md:px-8 relative z-10 text-center max-w-3xl mx-auto">
        <FiLock className="mb-6 text-red-500 animate-pulse" size={80} />
        <h1 className="text-4xl sm:text-5xl font-extrabold text-red-400 mb-4 animate-fadeInDown">
          Acesso Negado
        </h1>
        <p className="text-lg sm:text-xl text-gray-300 max-w-xl mb-8 animate-fadeInUp">
          Você não tem permissão para acessar esta página.
          Por favor, entre com uma conta que tenha os privilégios necessários.
        </p>
        <button
          onClick={() => navigate("/login")}
          className="px-6 py-3 bg-red-600 hover:bg-red-700 transition rounded-md text-white font-semibold shadow-lg animate-bounce"
        >
          Voltar para Login
        </button>
      </main>
